          ...
          if (!reg.test(this.form.wbid)) {
            ...
            axios.post(api.scrapyd,
              ...
              if (response.data.success) {
                this.scrapyd.jobId = (response.data.result && response.data.result.jobid) || '已提交'
                this.getScrapyd()
                this.$notify({
                  title: '启动持续爬虫成功',
      ...
      axios.get(api.scrapyd)
        .then((response) => {
          ...
          if (response.data.success && response.data.data) {
            ...
            this.scrapyd.status = '👌'
            this.scrapyd.name = d.node_name || '未知节点'
            this.scrapyd.finished = d.finished || 0
            this.scrapyd.pending = d.pending || 0
            this.scrapyd.running = d.running || 0
          } else {
            this.scrapyd.status = '❌ '
            this.scrapyd.name = response.data.message || '服务不可用'
          }
        })
        .catch((error) => {
          this.scrapyd.status = '❌'
          this.scrapyd.name = '连接失败'
