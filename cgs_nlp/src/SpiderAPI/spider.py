...
class Weibo:
    ...
    def get_userInfo(self):
        ...
        try:
            ...
            html = requests.get(url, cookies=self.cookie, headers=self.agent, timeout=self.timeout).content
            ...
            if image:
                user_info.Image = image[0] if len(image) > 0 else ''
            ...
            if place and len(place) > 0:
                ...
                user_info.Province = place[0] if len(place) > 0 else ''
                if len(place) > 1:
                    user_info.City = place[1]
            if briefIntroduction and len(briefIntroduction) > 0:
            ...
            if birthday and len(birthday) > 0:
                ...
                try:
                    ...
                    user_info.Birthday = birthday.strftime("%Y-%m-%d")
                except Exception:
                    user_info.Constellation = birthday[0]  # 可能是星座
            if sexOrientation and len(sexOrientation) > 0:
                if len(gender) > 0 and sexOrientation[0].replace(u"\xa0", "") == gender[0]:
            ...
            if vipLevel and len(vipLevel) > 0:
            ...
            try:
                ...
                r = requests.get(urlothers, headers=self.agent, cookies=self.cookie, timeout=self.timeout)
    ...
    def get_long_weibo(self, weibo_link):
        # noinspection PyBroadException
        try:
            html = requests.get(weibo_link, headers=self.agent, cookies=self.cookie, timeout=self.timeout).content
            selector = etree.HTML(html)
            c_divs = selector.xpath("div[@class='c']")
            if len(c_divs) < 2:
                return ''
            info = c_divs[1]
            m_divs = info.xpath('div[@id="M_"]/span[@class="ctt"]')
            if not m_divs:
                return ''
            wb_content = m_divs[0].xpath(
    ...
    def get_retweet(self, is_retweet, info, wb_content):
        ...
        try:
            ...
            info_divs = info.xpath("div")
            if not info_divs:
                return wb_content
            retweet_reason = info_divs[-1].xpath("string(.)").replace(u"\u200b", "").encode(
                sys.stdout.encoding, "ignore").decode(
                sys.stdout.encoding)
            if u"赞" not in retweet_reason:
                return wb_content
    ...
    def get_weibo_content(self, info):
        ...
        try:
            ...
            weibo_content = str_t[0].xpath("string(.)").replace(u"\u200b", "").encode(
                sys.stdout.encoding, "ignore").decode(
                sys.stdout.encoding)
            weibo_ids = info.xpath("@id")
            if not weibo_ids:
                return weibo_content
            weibo_id = weibo_ids[0][2:] if len(weibo_ids[0]) > 2 else ''
            a_link = info.xpath("div/span[@class='ctt']/a")  # 原始微博
            is_retweet = info.xpath("div/span[@class='cmt']")  # 转发微博
            if a_link:
                last_a = a_link[-1]
                text_nodes = last_a.xpath("text()")
                if text_nodes and len(text_nodes) > 0 and text_nodes[0] == u"全文":
                    weibo_link = "https://weibo.cn/comment/" + weibo_id
                    wb_content = self.get_long_weibo(weibo_link)
                    if wb_content:
                        if not is_retweet and len(wb_content) > 1:
    ...
    def get_weibo_place(self, info):
        try:
            div_list = info.xpath("div")
            if not div_list:
                return u"无"
            div_first = div_list[0]
            a_list = div_first.xpath("a")
            weibo_place = u"无"
            for a in a_list:
                href_list = a.xpath("@href")
                text_list = a.xpath("text()")
                if (href_list and len(href_list) > 0 and "place.weibo.com" in href_list[0] and
                        text_list and len(text_list) > 0 and text_list[0] == u"显示地图"):
                    weibo_a = div_first.xpath("span[@class='ctt']/a")
                    if len(weibo_a) >= 1:
                        weibo_place = weibo_a[-1]
                        ctt_text_list = div_first.xpath("span[@class='ctt']/a/text()")
                        if ctt_text_list and len(ctt_text_list) > 0 and u"的秒拍视频" in ctt_text_list[-1]:
                            if len(weibo_a) >= 2:
    ...
    def get_weibo_info(self):
        ...
        try:
            ...
            html = requests.get(url, cookies=self.cookie, headers=self.agent, timeout=self.timeout).content
            selector = etree.HTML(html)
            mp_inputs = selector.xpath("input[@name='mp']")
            if not mp_inputs:  # 判断是否为第一页
                page_num = 1
            else:
                page_num = (int)(mp_inputs[0].attrib.get("value", 1))
            ...
            for page in range(1, page_num + 1):
                ...
                html2 = requests.get(url2, headers=self.agent, cookies=self.cookie, timeout=self.timeout).content
                ...
                if is_empty:  # 判断是否有微博
                    for info in infos:
                        ...
                        if content:
                            ...
                            print(s.keywords(5))
                        if cooridinates:
                            tweetsItems.PubTools = cooridinates
                        if like:
    ...
    # 安全获取 XPath 结果的辅助函数
    def safe_xpath(self, selector, xpath_expr, index=0, default=''):
        try:
            results = selector.xpath(xpath_expr)
    ...
    def get_comment_info(self, id):
        ...
        wb_r = requests.get(wb_url, headers=self.agent, cookies=self.cookie, timeout=self.timeout).content
        soup = BeautifulSoup(wb_r, 'lxml')
        scripts = soup.select('body script')
        if not scripts:
            return "未找到脚本内容"
        src = scripts[0].string
        if not src:
            return "脚本内容为空"
        ...
        # 基本信息 - 使用安全的 XPath 访问
        wb_id = self.safe_xpath(selector2, "property[@name='id']/text()", 1, id)
        wb_userName = self.safe_xpath(selector2, "property[@name='screen_name']/string/text()", 0, '')
        wb_userId_raw = self.safe_xpath(selector2, "property[@name='profile_url']/text()", 1, '')
        try:
            if 'uid=' in wb_userId_raw:
                wb_userId = wb_userId_raw.split('uid=')[1].split('&')[0][0:10]
            else:
                wb_userId = id
        except (IndexError, Exception):
            wb_userId = id
        wb_user_profile_image_url = self.safe_xpath(selector2, "property[@name='profile_image_url']/text()", 1, '')
        wb_created_at = self.safe_xpath(selector2, "property[@name='created_at']/text()", 1, '')
        wb_source = self.safe_xpath(selector2, "property[@name='source']/text()", 1, '')
        wb_text = self.safe_xpath(selector2, "property[@name='text']/text()", 1, '')
        wb_pic_ids = selector2.xpath("property[@name='pic_ids']/array/string/text()") or []
        wb_reposts = self.safe_xpath(selector2, "property[@name='reposts_count']/@value", 0, '0')
        wb_comments = self.safe_xpath(selector2, "property[@name='comments_count']/@value", 0, '0')
        wb_like = self.safe_xpath(selector2, "property[@name='attitudes_count']/@value", 0, '0')
        ...
        if wb_pic_ids:
            ...
            for wb_pic_id in wb_pic_ids:
                with urllib.request.urlopen("https://wx2.sinaimg.cn/large/" + wb_pic_id, timeout=30) as response, open(
                        path.join(filepath, wb_pic_id + ".jpg"), 'wb') as f_save:
                    print("下载图片%s" % wb_pic_id)
                    f_save.write(response.read())
                    f_save.flush()
                    f_save.close()
        if wb_reposts:
            commentWeiboInfo.wb_repost_num = int(wb_reposts)
        if wb_comments:
            commentWeiboInfo.wb_comment_num = int(wb_comments)
        if wb_like:
            commentWeiboInfo.wb_like_num = int(wb_like)
        ...
        comment_num = 1
        while True:
            r = requests.get(url=c_urls.format(i), headers=self.agent, cookies=self.cookie, timeout=self.timeout)
            if int(r.json()['ok']) == 1:
                ...
                for j in range(0, len(comment_data)):
                    ...
                    if c_user_name:
                        commentInfo.c_userName = c_user_name
                    if c_user_img:
                        commentInfo.c_user_profile_image_url = c_user_img
                    if c_user_url:
                        commentInfo.c_user_profile_url = c_user_url
                    if c_text:
