...
class InputGroup(layout.Field):
    ...
    def render(self, form, context, template_pack=TEMPLATE_PACK, extra_context=None, **kwargs):
        ...
        if hasattr(self, 'wrapper_class'):
            field_context['wrapper_class'] = self.wrapper_class
        if extra_context:
            field_context.update(extra_context)
            
        return render_field(
            self.field, form, context, template=self.template,
            attrs=self.attrs, template_pack=template_pack, extra_context=field_context, **kwargs)
