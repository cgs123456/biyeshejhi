...
setattr(action_checkbox, "short_description", mark_safe(
    '<input type="checkbox" id="action-toggle" />'))
setattr(action_checkbox, "allow_tags", True)
setattr(action_checkbox, "allow_export", False)
setattr(action_checkbox, "is_column", False)
...
class ActionPlugin(BaseAdminPlugin):
    ...
    def response_action(self, ac, queryset):
        if isinstance(ac, type) and issubclass(ac, BaseActionView):
            ...
            return ac(self.admin_view, self.admin_view.request, queryset)
    ...
    def get_action_choices(self):
        ...
        """
        choices = []
        for ac, name, description, icon in (self.actions or OrderedDict()).values():
    ...
    def get_action(self, action):
        ...
        short_description = getattr(func, 'short_description', None)
