...
@author: LAB_ADM
'''
# pyright: reportIncompatibleMethodOverride=false, reportAttributeAccessIssue=false, reportArgumentType=false, reportGeneralTypeIssues=false
...
QUERY_TERMS = {'exact', 'iexact', 'contains', 'icontains', 'in', 'gt', 'gte', 'lt', 'lte', 'startswith', 'istartswith', 'endswith', 'iendswith', 'range', 'year', 'month', 'day', 'isnull', 'search', 'regex', 'iregex'}
...
class QuickFilterPlugin(BaseAdminPlugin):
    ...
    def get_list_queryset(self, queryset):
        lookup_params = dict([(force_str(k)[len(FILTER_PREFIX):], v) for k, v in self.admin_view.params.items() if force_str(k).startswith(FILTER_PREFIX) and v != ''])
        ...
        self.admin_view.quickfilter['clean_query_url'] = self.admin_view.get_query_string(remove=[k for k in self.admin_view.request.GET.keys() if k.startswith(FILTER_PREFIX)])
        ...
        if self.list_quick_filter:
            for list_quick_filter in self.list_quick_filter:
                ...
                if not isinstance(field, models.Field):
                    ...
                    field = field_parts[-1]
                spec = QuickFilterMultiSelectFieldListFilter(field, self.admin_view.request, lookup_params, self.model, self.admin_view, field_path=field_path,
        ...
        obj = list(filter(lambda f: f.is_used, self.filter_specs))
