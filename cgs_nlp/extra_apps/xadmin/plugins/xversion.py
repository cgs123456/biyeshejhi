# pyright: reportAttributeAccessIssue=false, reportIncompatibleMethodOverride=false, reportGeneralTypeIssues=false, reportOptionalMemberAccess=false, reportOptionalSubscript=false
...
from xadmin.layout import render_field
...
class ActionRevisionPlugin(BaseAdminPlugin):
    ...
    def do_action(self, __, queryset):
        with do_create_revision(self.admin_view.request):
            return __()
    ...
    def get_version_object(self, version):
        ...
        for field_name, pks in obj_version.m2m_data.items():
            ...
            if f.remote_field and isinstance(f.remote_field, models.ManyToManyRel):
                setattr(obj, f.name, f.remote_field.model._default_manager.get_queryset(
...
class InlineRevisionPlugin(BaseAdminPlugin):
    ...
    def _hack_inline_formset_initial(self, revision_view, formset):
        ...
        if self.admin_view.request.method == 'GET' and formset.helper and formset.helper.layout:
