# pyright: reportIncompatibleMethodOverride=false, reportAttributeAccessIssue=false, reportArgumentType=false
import calendar
import datetime
import decimal
        ...
        if hasattr(model_admin, 'data_charts'):
            if chart and chart in model_admin.data_charts:
                ...
                if self.title is None:
                    ...
                    self.title = gettext(
                        "%s Charts") % model_meta.verbose_name_plural
...
class ChartsView(ListAdminView):
    ...
    def get(self, request, name):
        ...
        return HttpResponse(result.encode('utf-8'), content_type='application/json; charset=utf-8')
