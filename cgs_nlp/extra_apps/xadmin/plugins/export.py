# pyright: reportMissingImports=false, reportAttributeAccessIssue=false, reportIncompatibleMethodOverride=false, reportGeneralTypeIssues=false, reportArgumentType=false
import io
import datetime
import sys
...
class ExportPlugin(BaseAdminPlugin):
    ...
    def init_request(self, *args, **kwargs):
        return self.admin_view.request.GET.get('_do_') == 'export'

    def _format_value(self, o):
        if (o.field is None and getattr(o.attr, 'boolean', False)) or \
           (o.field and isinstance(o.field, (BooleanField, BooleanField))):
                value = o.value
    ...
    def get_xlsx_export(self, context):
        ...
        output = io.BytesIO()
        export_header = (
            self.admin_view.request.GET.get('export_xlsx_header', 'off') == 'on')
    ...
    def get_xls_export(self, context):
        ...
        output = io.BytesIO()
        export_header = (
            self.admin_view.request.GET.get('export_xls_header', 'off') == 'on')
    ...
    def get_csv_export(self, context):
        ...
        if self.admin_view.request.GET.get('export_csv_header', 'off') != 'on':
            datas = datas[1:]
    ...
    def get_json_export(self, context):
        ...
        return json.dumps({'objects': results}, ensure_ascii=False,
                          indent=(self.admin_view.request.GET.get('export_json_format', 'off') == 'on') and 4 or None)

    def get_response(self, response, context, *args, **kwargs):
        file_type = self.admin_view.request.GET.get('export_type', 'csv')
    ...
    def get_result_list(self, __):
        if self.admin_view.request.GET.get('all', 'off') == 'on':
