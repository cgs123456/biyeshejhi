# pyright: reportMissingImports=false, reportAttributeAccessIssue=false, reportIncompatibleMethodOverride=false, reportGeneralTypeIssues=false
import xadmin
...
class CommentsAdmin(object):
    ...
    setattr(flag_comments, "short_description", _("Flag selected comments"))
    setattr(flag_comments, "icon", "flag")

    def approve_comments(self, request, queryset):
        self._bulk_flag(queryset, perform_approve,
                        lambda n: ngettext('approved', 'approved', n))
    setattr(approve_comments, "short_description", _("Approve selected comments"))
    setattr(approve_comments, "icon", "ok")

    def remove_comments(self, request, queryset):
        self._bulk_flag(queryset, perform_delete,
                        lambda n: ngettext('removed', 'removed', n))
    setattr(remove_comments, "short_description", _("Remove selected comments"))
    setattr(remove_comments, "icon", "remove-circle")
...
if 'django_comments' in settings.INSTALLED_APPS and get_model and Comment and (get_model() is Comment):
