# pyright: reportMissingImports=false, reportIncompatibleMethodOverride=false, reportAttributeAccessIssue=false, reportGeneralTypeIssues=false, reportOptionalCall=false, reportArgumentType=false
import re
...
class WizardFormPlugin(BaseAdminPlugin):
    ...
    def init_request(self, *args, **kwargs):
        ...
        if request.headers.get('x-requested-with') == 'XMLHttpRequest' or ("_ajax" in request.GET) or not hasattr(request, 'session') or (args and not self.wizard_for_update):
            # update view
            return False
    ...
    def prepare_form(self, __):
        ...
        if not get_storage or not StepsHelper:
            return
        self.storage = get_storage(
            self.storage_name, self.prefix, self.admin_view.request,
            getattr(self, 'file_storage', None))
        self.steps = StepsHelper(self)
        self.wizard_goto_step = False

        if self.admin_view.request.method == 'GET':
            self.storage.reset()
            ...
            wizard_goto_step = self.admin_view.request.POST.get('wizard_goto_step', None)
            ...
            management_form = ManagementForm(
                self.admin_view.request.POST, prefix=self.prefix)
    ...
    def get_form_datas(self, datas):
        ...
        if self.admin_view.request.method == 'POST' and self.wizard_goto_step:
            datas.update({
