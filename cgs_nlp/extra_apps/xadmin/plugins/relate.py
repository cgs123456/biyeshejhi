# coding=UTF-8
# pyright: reportAttributeAccessIssue=false, reportIncompatibleMethodOverride=false, reportGeneralTypeIssues=false, reportOptionalMemberAccess=false
...
class RelateMenuPlugin(BaseAdminPlugin):
    ...
    setattr(related_link, "short_description", '&nbsp;')
    setattr(related_link, "allow_tags", True)
    setattr(related_link, "allow_export", False)
    setattr(related_link, "is_column", False)
...
class BaseRelateDisplayPlugin(BaseAdminPlugin):
    ...
    def init_request(self, *args, **kwargs):
        ...
        for k, v in self.admin_view.request.GET.items():
            key = force_str(k)
            if key.startswith(RELATE_PREFIX):
                self.relate_obj = RelateObject(
                    self.admin_view, key[len(RELATE_PREFIX):], v)
                break
