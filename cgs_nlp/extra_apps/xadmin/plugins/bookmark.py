# pyright: reportAttributeAccessIssue=false, reportIncompatibleMethodOverride=false, reportGeneralTypeIssues=false
...
class BookmarkPlugin(BaseAdminPlugin):
    ...
    def get_context(self, context):
        ...
        bk_meta = getattr(Bookmark, "_meta")
        bk_model_info = (bk_meta.app_label, bk_meta.model_name)
        bookmarks_queryset = Bookmark._default_manager.filter(
...
class BookmarkAdmin(object):
    ...
    def queryset(self):
        if self.user.is_superuser:
            return Bookmark._default_manager.all()
        return Bookmark._default_manager.filter(Q(user=self.user) | Q(is_share=True))
...
class BookmarkWidget(PartialBaseWidget):
    ...
    bookmark = ModelChoiceField(
        label=_('Bookmark'), queryset=Bookmark._default_manager.all(), required=False)
