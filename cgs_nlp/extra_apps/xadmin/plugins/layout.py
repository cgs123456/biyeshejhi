...
class GridLayoutPlugin(BaseAdminPlugin):
    ...
    def get_layout(self, l):
        item = l if isinstance(l, dict) else DEFAULT_LAYOUTS[str(l)]
        return dict({'url': self.admin_view.get_query_string({LAYOUT_VAR: item['key']}), 'selected': False}, **item)

    def init_request(self, *args, **kwargs):
        active = bool(self.admin_view.request.method == 'GET' and self.grid_layouts)
        if active:
            ...
            self._current_layout = self.admin_view.request.GET.get(LAYOUT_VAR, self._active_layouts[0]['key'])
