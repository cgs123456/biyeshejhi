# pyright: reportAttributeAccessIssue=false, reportIncompatibleMethodOverride=false, reportGeneralTypeIssues=false, reportArgumentType=false, reportOptionalMemberAccess=false
import operator
...
class FilterPlugin(BaseAdminPlugin):
    ...
    def get_list_queryset(self, queryset):
        lookup_params = dict([(force_str(k)[len(FILTER_PREFIX):], v) for k, v in self.admin_view.params.items()
                              if force_str(k).startswith(FILTER_PREFIX) and v != ''])
        ...
        self.admin_view.clean_query_url = self.admin_view.get_query_string(remove=[k for k in self.admin_view.request.GET.keys() if
        ...
        if self.list_filter:
            for list_filter in self.list_filter:
                if callable(list_filter):
                    ...
                    spec = list_filter(self.admin_view.request, lookup_params,
                                       self.model, self)
                else:
                    field_path = None
                    field_parts = []
                    ...
                    spec = field_list_filter_class(
                        field, self.admin_view.request, lookup_params,
        ...
        try:
            ...
            if isinstance(queryset, QuerySet) and lookup_params:
        ...
        query = self.admin_view.request.GET.get(SEARCH_VAR, '')
