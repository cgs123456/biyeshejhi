#!/usr/bin/env python
# encoding=utf-8
# pyright: reportAttributeAccessIssue=false, reportIncompatibleMethodOverride=false, reportArgumentType=false, reportCallIssue=false, reportGeneralTypeIssues=false, reportOptionalMemberAccess=false, reportOptionalCall=false
"""
Author:zcyuefan
...
class ImportBaseView(ModelAdminView):
    ...
    def get_tmp_storage_class(self):
        if self.tmp_storage_class is None:
            return getattr(settings, "IMPORT_EXPORT_TMP_STORAGE_CLASS", TempFolderStorage)
        else:
...
class ExportPlugin(ExportMixin, BaseAdminPlugin):
    ...
    def init_request(self, *args, **kwargs):
        return self.admin_view.request.GET.get('_action_') == 'export'
    ...
    def get_response(self, response, context, *args, **kwargs):
        ...
        export_format = self.admin_view.request.GET.get('file_format')

        if not export_format:
            messages.warning(self.admin_view.request, _('You must select an export format.'))
        else:
            formats = self.get_export_formats()
            file_format = formats[int(export_format)]()
            queryset = self.get_export_queryset(self.admin_view.request, context)
            export_data = self.get_export_data(file_format, queryset, request=self.admin_view.request)
