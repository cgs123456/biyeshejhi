...
class EditablePlugin(BaseAdminPlugin):
    ...
    def init_request(self, *args, **kwargs):
        active = bool(self.admin_view.request.method == 'GET' and self.admin_view.has_change_permission() and self.list_editable)
        if active:
...
class EditPatchView(ModelFormAdminView, ListAdminView):
    ...
    def get(self, request, object_id):
        ...
        c = template.Context({'form': form, 'action_url': self.model_admin_url('patch', cast(Any, self.org_obj).pk)})

        return HttpResponse(t.render(c).encode('utf-8'))
