...
class ThemePlugin(BaseAdminPlugin):
    ...
    def _get_theme(self):
        if self.admin_view.user:
            try:
                return UserSettings._default_manager.get(user=self.admin_view.user, key="site-theme").value
            except Exception:
                pass
        if '_theme' in self.admin_view.request.COOKIES:
            return unquote(self.admin_view.request.COOKIES['_theme'])
            ...
            if ex_themes:
                ...
                try:
                    ...
                    resp, content = h.request("https://bootswatch.com/api/3.json", 'GET', '',
                                              headers={"Accept": "application/json", "User-Agent": self.admin_view.request.META['HTTP_USER_AGENT']})
