...
class QuickFormPlugin(BaseAdminPlugin):
    ...
    def init_request(self, *args, **kwargs):
        ...
        if (request.method == 'GET' and request.headers.get('x-requested-with') == 'XMLHttpRequest') or request.GET.get('_ajax'):
    ...
    def get_model_form(self, __, **kwargs):
        request = self.admin_view.request
        if '_field' in request.GET:
            defaults = {
                "form": self.admin_view.form,
                "fields": request.GET['_field'].split(','),
    ...
    def get_form_layout(self, __):
        request = self.admin_view.request
        if '_field' in request.GET:
            return Layout(*request.GET['_field'].split(','))
        return __()

    def get_context(self, context):
        context['form_url'] = self.admin_view.request.path
        return context
...
class RelatedFieldWidgetWrapper(forms.Widget):
    ...
    def build_attrs(self, base_attrs=None, extra_attrs=None):
        ...
        self.attrs = self.widget.build_attrs(base_attrs, extra_attrs=extra_attrs)
        return self.attrs
