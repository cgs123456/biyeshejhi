...
class ForeignKeySelectWidget(ForeignKeySearchWidget):
    ...
    def build_attrs(self, base_attrs=None, extra_attrs=None):
        attrs = super(ForeignKeySelectWidget, self).build_attrs(base_attrs, extra_attrs=extra_attrs)
        if "class" not in attrs:
...
class RelateFieldPlugin(BaseAdminPlugin):
    ...
    def get_field_style(self, attrs, db_field, style, **kwargs):
        ...
        if style in ('fk-ajax', 'fk-select') and isinstance(db_field, models.ForeignKey) and db_field.remote_field:
            rel_model = db_field.remote_field.model
            if (rel_model in self.admin_view.admin_site._registry) and \
                    self.has_model_perm(rel_model, 'view'):
                db = kwargs.get('using')
