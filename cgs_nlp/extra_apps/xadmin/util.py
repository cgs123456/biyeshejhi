...
class NestedObjects(Collector):
    ...
    def related_objects(self, related_model, related_fields, objs):
        qs = super(NestedObjects, self).related_objects(related_model, related_fields, objs)
        if related_fields:
            field_name = getattr(related_fields[0], 'name', None)
            if field_name:
...
def model_format_dict(obj):
    ...
    if isinstance(obj, QuerySet):
        opts = getattr(obj.model, '_meta', obj)
    elif isinstance(obj, (models.Model, ModelBase)):
        opts = getattr(obj, '_meta', obj)
    elif hasattr(obj, '_meta'):
        opts = obj._meta
    else:
        opts = obj
    verbose_name = force_str(getattr(opts, 'verbose_name', ''))
    verbose_name_plural = force_str(getattr(opts, 'verbose_name_plural', verbose_name))
    return {
...
def is_rel_field(name, model):
    if hasattr(name, 'split') and name.find("__") > 0:
        parts = name.split("__")
        if parts[0] in model._meta.get_fields():
            return True
    return False
...
def display_for_field(value, field):
    ...
    # before the general null test.
    elif isinstance(field, models.BooleanField) or isinstance(field, models.BooleanField):
        ...
        return ', '.join(force_str(obj) for obj in value.all())
    else:
...
def get_limit_choices_to_from_path(model, path):
    ...
    if not limit_choices_to:
        ...
        # Ensure kwargs keys are str for static typing and runtime safety.
        return models.Q(**{str(k): v for k, v in dict(limit_choices_to).items()})
