...
# Python 3 only runtime; legacy PY2 encoding branch removed.
...
class AdminSite(object):
    ...
    def register_modelview(self, path, admin_view_class, name):
        ...
        if issubclass(admin_view_class, BaseAdminView):
            ...
            base_admin_view_name = getattr(BaseAdminView, "__name__", "BaseAdminView")
            raise ImproperlyConfigured(u'The registered view class %s isn\'t subclass of %s' %
                                       (admin_view_class.__name__, base_admin_view_name))
    ...
    def register(self, model_or_iterable, admin_class=object, **options):
        ...
        for model in model_or_iterable:
            if isinstance(model, ModelBase):
                ...
                if model_meta is not None and getattr(model_meta, "abstract", False):
                ...
                admin_class = type(str("%s%sAdmin" % (app_label, model_name)), (admin_class,), options or {})
                setattr(admin_class, "model", model)
                setattr(admin_class, "order", self.model_admins_order)
        ...
        for model in model_or_iterable:
            if isinstance(model, ModelBase):
                ...
                if model not in self._registry_avs:
                    ...
                    raise NotRegistered('The admin_view_class %s is not registered' % model_name)
    ...
    def check_dependencies(self):
        ...
        content_type_meta = getattr(ContentType, "_meta", None)
        if not getattr(content_type_meta, "installed", True):
