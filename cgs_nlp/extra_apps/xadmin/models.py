...
class Bookmark(models.Model):
    ...
    is_share = models.BooleanField(verbose_name=_(u'Is Shared'), default=cast(Any, False))
...
class JSONEncoder(DjangoJSONEncoder):
    ...
    def default(self, o):
        if isinstance(o, datetime.datetime):
            ...
            model_meta = getattr(o, "_meta", None)
...
class UserWidget(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if created:
            try:
                portal_pos = UserSettings._default_manager.get(
...
class Log(models.Model):
    ...
    def __repr__(self) -> str:
        return str(smart_str(self.action_time))

    def __str__(self) -> str:
        if self.action_flag == 'create':
            return str(gettext('Added "%(object)s".') % {'object': self.object_repr})
        elif self.action_flag == 'change':
            return str(gettext('Changed "%(object)s" - %(changes)s') % {
                'object': self.object_repr,
                'changes': self.message,
            })
        elif self.action_flag == 'delete' and self.object_repr:
            return str(gettext('Deleted "%(object)s."') % {'object': self.object_repr})
    ...
    def get_edited_object(self):
        ...
        if content_type is None:
            return None
        return cast(Any, content_type).get_object_for_this_type(pk=self.object_id)
