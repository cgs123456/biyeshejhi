jQuery(function() {
  $( ".btn-quick-form" ).click(function() {
    ...
    if (!btn.data('form-modal')) {
      var modal = $('<div class="modal fade quick-form" role="dialog"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button><h3>' + 
        btn.attr('title') + '</h3></div><div class="modal-body"></div>' +
        '<div class="modal-footer"><button class="btn btn-default" data-dismiss="modal" aria-hidden="true">' + (typeof gettext !== 'undefined' ? gettext('Close') : 'Close') + '</button>' +
        '<a class="btn btn-primary btn-submit">' + (typeof gettext !== 'undefined' ? gettext('Save changes') : 'Save changes') + '</a></div></div>');
      $('body').append(modal);
      btn.data('form-modal', modal);
      modal.find('.modal-body').load(form_url, function(status, xhr) {
            ...
            success: function(data) {
              if (data['result'] !== 'success' && data['errors']) {
                ...
                if (non_fields_errors.length) {
                  var err_html = [];
                  for (var k = non_fields_errors.length - 1; k >= 0; k--) {
                    err_html.push('<p class="text-error"><strong>' + non_fields_errors[k] + '</strong></p>');
                  }
